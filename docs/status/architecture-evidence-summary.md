# Architecture Evidence Summary

Generated at: 2026-05-25T01:45:43.965Z

## Counts

- Nodes: 452
- Relations: 761
- Chains: 34
- Test mappings: 8
- Evidence rows: 452

## Verification Status

- tested: 386
- verified: 66

## Evidence Status

- tested: 386
- verified: 66

## Missing Evidence Queue

- No missing evidence recorded.

## Drift Detection

- API Routes: missing_in_registry=0, missing_in_code=0
- Prisma Models: missing_in_registry=0, missing_in_code=0
- React App Routes: missing_in_registry=0, missing_in_code=0
- Script/Test Commands: missing_in_registry=0, missing_in_code=0

Detailed report: `docs/status/architecture-drift-report.md`

Scaffold queue: `docs/status/architecture-drift-missing-scaffold.csv`

## Obsidian Entry Points

- [[FEAT-ARCH-EVIDENCE-SYSTEM|Architecture Evidence System]]
- [[FEAT-MGMT-DEPT-CATALOG|Management Department Catalog]]
- [[FEAT-OPERATIONS-WORK-ITEMS|Operations Work Items]]
- [[FEAT-DASHBOARD-COMMAND|Dashboard Command Packet]]
